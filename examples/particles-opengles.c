...
static void draw_particles(GLFWwindow* window, double t, float dt)
{
    ...
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), &vertex_array[0].s);
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(1, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(Vertex), &vertex_array[0].rgba);
    glEnableVertexAttribArray(1);
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), &vertex_array[0].x);
    ...
    for (i = 0;  i < MAX_PARTICLES;  i++)
    {
        ...
        // of quads using glDrawArrays.
            glDrawArrays(GL_TRIANGLE_STRIP, 0, PARTICLE_VERTS * particle_count);
        {
